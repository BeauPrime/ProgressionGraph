(() => {
  ...
  var sprintf_regex = /(?=[\b\W])%(\d+)/g;
  function sprintf_replacer(...args) {
    return (substring, ...captures) => {
      const idx = parseInt(captures[0]) - 1;
      return idx < 0 || idx >= args.length ? "" : toString(args[idx]);
    };
  }
  function toString(obj) {
    return obj !== null && obj !== void 0 ? obj.toString() : null;
  }
  ...
  ((List2) => {
    ...
    List2.FastRemoveAt = FastRemoveAt;
    ...
    List2.FastRemove = FastRemove;
    function Has(array, value) {
      return array.indexOf(value) >= 0;
    }
    List2.Has = Has;
    function ValueSort(array) {
      array.sort(DefaultCompare);
    }
    List2.ValueSort = ValueSort;
  })(List || (List = {}));
  function RandomRangeI(minOrMax, max) {
    if (IsUndefined(max)) {
      max = minOrMax;
      minOrMax = 0;
    }
  ...
  function Shuffle(array, offset, count) {
    offset = ValueOrDefault(offset, 0);
    count = ValueOrDefault(count, array.length);
    let i = Math.min(offset + count, array.length);
    let j;
    while (--i > offset) {
      const old = array[i];
  ...
  var TaskQueue = class {
    ...
    Schedule(method, name, onComplete) {
      return this.ScheduleWithContext(method, void 0, name, onComplete);
    }
    Priority(method, name, onComplete) {
      return this.PriorityWithContext(method, void 0, name, onComplete);
    }
    ScheduleWithContext(method, context, name, onComplete) {
      const task = {
        name,
        id: this.NextId(),
    ...
    PriorityWithContext(method, context, name, onComplete) {
      const task = {
        name,
        id: this.NextId(),
        type: TypeOfMethod(method),
    ...
    Tick(milliseconds) {
      ...
      while (this._workUnits.length > 0 && (remaining = start + milliseconds - performance.now()) > 0) {
        ...
        top.status = 2 /* EXECUTING */;
        switch (top.type) {
          case 0 /* FUNCTION */: {
            const method = top.method;
            while ((remaining = start + milliseconds - performance.now()) > 0) {
              const result = method.apply(void 0, top.context);
              if (!result) {
                top.status = 5 /* FINISHED */;
                List.Remove(this._workUnits, top);
                top.onComplete && top.onComplete(top.name);
                break;
              } else if (top.status != 2 /* EXECUTING */) {
                break;
              } else if (result == 2 /* NEXT_FRAME */) {
                return;
              }
            }
            break;
          ...
          case 1 /* GENERATOR */: {
            const generator = top.method;
            while ((remaining = start + milliseconds - performance.now()) > 0) {
              const result = generator.next(top.context);
              if (result.done || result.value !== void 0 && !result.value) {
                top.status = 5 /* FINISHED */;
                List.Remove(this._workUnits, top);
                top.onComplete && top.onComplete(top.name);
                break;
                ...
                break;
              } else if (result.value == 2 /* NEXT_FRAME */) {
                return;
              }
            }
            break;
    ...
    FlushTop() {
      ...
      switch (top.type) {
        case 0 /* FUNCTION */: {
          const method = top.method;
          let hasMore;
          while (hasMore = method.apply(void 0, top.context)) {
            if (top.status == 0 /* INVALID */) {
              return true;
            } else {
              top.status == 2 /* EXECUTING */;
            }
    ...
    Cancel(taskId) {
      ...
      if (idx >= 0) {
        this._workUnits[idx].status = 0 /* INVALID */;
    ...
    Clear() {
      for (let i = 0, len = this._workUnits.length; i < len; i++) {
        this._workUnits[i].status = 0 /* INVALID */;
      }
  ...
  function ProcessContextIntoMethodArgs(context) {
  ...
  // src/Config.ts
  var UnlockTraverse = "traverse";
  var UnlockManual = "manual";
  var UnlockAuto = "auto";
  ...
  function ApplyDefaultsToNode(node, id) {
  ...
  // src/Modifiers.ts
  function FillModifierDefaults(modifiers2, config) {
    modifiers2.tokenAddMultipliers = modifiers2.tokenAddMultipliers || {};
    modifiers2.tokenConsumeMultipliers = modifiers2.tokenConsumeMultipliers || {};
    for (let i = 0, len = config.tokenIds.length; i < len; i++) {
      const id = config.tokenIds[i];
      AssignDefault(modifiers2.tokenAddMultipliers, id, 1);
      AssignDefault(modifiers2.tokenConsumeMultipliers, id, 1);
    }
  }
  function CopyModifiers(source, target) {
    if (!target) {
  ...
  var TraversalState = class {
    constructor() {
      this.name = null;
      this.unlocked = /* @__PURE__ */ new Set();
      this.status = {};
      this.addedTokens = {};
      this.consumedTokens = {};
  ...
  function TraversalReset(state, config, name = null, modifiers2, flags = 0) {
    ...
    if (modifiers2) {
      CopyModifiers(modifiers2, state.modifiers);
    }
    FillModifierDefaults(state.modifiers, config);
    ScanForVisible(state, config, null, flags);
  }
  function TraversalPerformStep(state, config, flags = 0) {
  ...
  function GetMissingRequirements(state, config, id) {
  ...
  function ScanForVisible(state, config, auto, flags = 0) {
    let changed = 0;
    for (let i = state.hidden.length - 1; i >= 0; i--) {
      ...
      flags & 1 /* DEBUG */ && report("[traversal %1] node %2 became available", state.name, id);
    ...
    for (let i = state.available.length - 1; i >= 0; i--) {
      ...
      if (!SatisfiesRequirements(state, node)) {
        ...
        flags & 1 /* DEBUG */ && report("[traversal %1] node %2 became unavailable", state.name, id);
        changed++;
      }
    }
    return changed;
  }
  function SumAvailableNodesByType(state, config, counters) {
  ...
  function Visit(state, config, visitQueue, rootId, step, flags = 0) {
    const hadStep = !!step;
    step = step || {
      id: rootId,
      available: {}
    ...
    while (visitQueue.length > 0) {
      ...
      flags & 1 /* DEBUG */ && report("[traversal %1] node %2 visited!", state.name, nodeId);
  ...
  function SatisfiesRequirements(state, node) {
    for (let i = 0, len = node.requires.length; i < len; i++) {
      const req = node.requires[i];
      if (typeof req.amount == "boolean") {
        if (state.status[req.id] != req.amount) {
  ...
  function ApplyRequirements(state, node, step) {
    for (let i = 0, len = node.requires.length; i < len; i++) {
      const req = node.requires[i];
      if (req.consume) {
  ...
  function ApplyResults(state, node, step, visitQueue) {
    for (let i = 0, len = node.results.length; i < len; i++) {
      ...
      if (res.consume) {
        ...
        if (changed != 0 /* NO_CHANGE */) {
          AddChangeToStep(step, res.id, val);
          if (changed == 2 /* NEW_ASSET */) {
  ...
  function ResolveValue(state, id, amount, forceConsume) {
    if (typeof amount == "boolean") {
      return amount;
    } else {
  ...
  function ChangeStatus(state, id, amount) {
    const oldVal = state.status[id];
    if (typeof amount == "boolean") {
      if (oldVal != amount) {
        state.status[id] = amount;
        return amount ? 2 /* NEW_ASSET */ : 3 /* REMOVED_ASSET */;
      } else {
        return 0 /* NO_CHANGE */;
      }
    } else {
  ...
  function AddChangeToStep(step, id, amount, unlock) {
  ...
  function LogInitialize() {
    ...
    frameElement = document.getElementById("page-body-frame");
  }
  function LogReset() {
    logElement.innerHTML = "";
    queuedLogLines.length = 0;
  }
  function LogWrite(msg, ...args) {
  ...
  function AggregateTraversal(aggregator, state) {
    for (let i = 0, len = state.path.length; i < len; i++) {
      ...
      for (const [key2, val] of Object.entries(step.available)) {
        AddKeyedCounter(aggregator.open, key2, val);
      }
    ...
    for (const [key, value] of Object.entries(state.addedTokens)) {
      AddKeyedCounter(aggregator.addedTokens, key, value);
    }
    for (const [key, value] of Object.entries(state.consumedTokens)) {
      AddKeyedCounter(aggregator.consumedTokens, key, value);
    }
    aggregator.sampleCount++;
  }
  function AggregateProcess(aggregator) {
  ...
  function Average(counters, statistics, sampleCount) {
  ...
  function AddKeyedCounter(map, id, value = 1) {
  ...
  OnPageLoad(() => {
    ...
    configButton = document.getElementById("button-config");
  ...
  function OnRunClicked() {
    ...
    TraversalReset(state, loadedConfig, null, modifiers, 1 /* DEBUG */);
  ...
  function RunStep(state) {
    ...
    const step = TraversalPerformStep(state, loadedConfig, 1 /* DEBUG */);
    if (step != null) {
  ...
  function* RunAggregateTrial(count, modifiers2) {
    const state = new TraversalState();
  ...
  function OnConfigurationRead(txt) {
    let config;
    try {
